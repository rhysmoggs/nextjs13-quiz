...
export default function Home() {
  ...
  const getQuizAPI = async () => {
    ...
    const new_questions = results.map((q) => {
      return q.question
    })
    console.log(new_questions)
    const new_correct_answers = results.map((q) => {
      // return q
      return q.correct_answer
    })
    console.log(new_correct_answers)
    const new_inccorrect_answers = results.map((q) => {
      return q.incorrect_answers.map((i) => {
        return i
      })
    })
    console.log(new_inccorrect_answers)

    //add all answers together:
    // const answers = [...new_inccorrect_answers, new_correct_answers]
    // console.log(answers)
